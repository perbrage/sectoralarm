...
module.exports = class Site {
    ...
    login() {
        return Promise.resolve()
            .then(() => client.getMetadata())
            .then(metadata => {
                this._version = metadata.version;
                return client.login(this._email, this._password, metadata.cookie);
            })
            .then(sessionCookie => {
    ...
    info() {
        return Promise.resolve()
            .then(() => client.getStatus(this._siteId, this._sessionCookie, this._version))
    ...
    temperatures(sensorId) {
        return Promise.resolve()
            .then(() => client.getTemperatures(this._siteId, this._sessionCookie, this._version))
            .then(temperatures => parser.transformTemperaturesToOutput(temperatures, sensorId))
    ...
    locks(lockId) {
        return Promise.resolve()
            ...
            .then(locks => parser.transformLocksToOutput(locks, lockId))
    ...
    smartPlugs(smartPlugId) {
        return Promise.resolve()
            .then(() => client.getSmartPlugs(this._siteId, this._sessionCookie))
            .then(plugs => JSON.parse(plugs))
            .then(plugs => this.formatOutput(plugs));
    }
