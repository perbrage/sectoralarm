...
class Client {
    ...
    getLocks(siteId, sessionCookie, retry = this._settings.numberOfRetries) {
        var client = this;
        var content = '';
        ...
        return new Promise(function(resolve, reject) {
            const requestWithRetry = (siteId, sessionCookie, retry) => {
                var request = https.request(options, async function(response) {
                    ...
                    if (response.statusCode == 401) {
                        if (retry != 0) {
                            await client.snooze(client._settings.retryDelayInMs);
                            retry--;
                            requestWithRetry(siteId, sessionCookie, retry);
                            return;
                        } else {
                            reject(new SectorAlarmError('ERR_INVALID_SESSION', 'Invalid session, please re-login'));
                        }
            ...
            return requestWithRetry(siteId, sessionCookie, retry);
        });
    }

    getHistory(siteId, sessionCookie, retry = this._settings.numberOfRetries) {
        var client = this;
        var content = '';
    ...
    getTemperatures(siteId, sessionCookie, version, retry = this._settings.numberOfRetries) {
        var client = this;
        var payload = JSON.stringify({
            "id": siteId,
            "Version": version
    ...
    actOnLock(siteId, lockId, sessionCookie, code, command, retry = this._settings.numberOfRetries) {
        var client = this;
        var content = '';
        var payload = JSON.stringify({
            "id": siteId,
        ...
        return new Promise(function(resolve, reject) {
            ...
            const requestWithRetry = (siteId, lockId, sessionCookie, code, command, retry) => {
                ...
                var request = https.request(options, async function(response) {
                    ...
                    if (response.statusCode == 401) {
                        if (retry != 0) {
                            await client.snooze(client._settings.retryDelayInMs);
                            retry--;
                            requestWithRetry(siteId, lockId, sessionCookie, code, command, retry);
                            return;
                        } else {
                            reject(new SectorAlarmError('ERR_INVALID_SESSION', 'Invalid session, please re-login'));
                        }
            ...
            return requestWithRetry(siteId, lockId, sessionCookie, code, command, retry);
        });
    }
};
