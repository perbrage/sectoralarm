...
class Client {
    ...
    getStatus(siteId, sessionCookie, version, retry = 2) {
        var client = this;
        var content = '';
            ...
            const requestWithRetry = (siteId, sessionCookie, version, retry) => {
                ...
                var request = https.request(options, async function(response) {
                    ...
                    if (response.statusCode == 401) {
                        if (retry != 0) {
                            await client.snooze(2000);
                            console.log('Snoozed for 2 seconds');
                            retry--;
        ...
        //                 console.log('Snoozed for 2 seconds');
        //                 retry--;
        //                 client.getStatus(siteId, sessionCookie, version, retry)
        //                     .then(result => { 
        //                         resolve(result);
        //                     })
        //                     .catch(error => {
